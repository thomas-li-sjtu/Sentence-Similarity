...
from transformers import AutoModel, AutoConfig, AutoTokenizer, DataCollatorWithPadding, AdamW, \
...
parser.add_argument('--pretrained_model_dir', type=str, help='预训练模型地址', default='./pretrained_model/roberta-base')
...
parser.add_argument('--save_steps', type=int, help='多少步保存模型', default=200)
...
parser.add_argument('--output_model_dir', type=str, help='模型保存地址', default='./roberta-finetune')
...
class CustomModel(nn.Module):
    def __init__(self, pretrain_model_path, model_param_config):
        ...
        self.config = AutoConfig.from_pretrained(pretrain_model_path, output_hidden_states=True)
        self.bert = AutoModel.from_pretrained(pretrain_model_path)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, labels=None, loss_fn=None):
...
vocab_file_dir = './pretrained_model/roberta-base'
tokenizer = AutoTokenizer.from_pretrained(vocab_file_dir)
