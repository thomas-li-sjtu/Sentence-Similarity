...
parser.add_argument('--pretrained_model_dir', type=str, help='预训练模型地址', default='./pretrained_model/sentence-bert')
...
parser.add_argument('--output_model_dir', type=str, help='模型保存地址', default='./bert-sentence')
...
class CustomModel(nn.Module):
    ...
    def mean_pooling(self, model_output, attention_mask):
        token_embeddings = model_output  # First element of model output contains all token embeddings
...
def tokenize_function(example):
    # for i, j in zip(example["text_a"], example["text_b"]):
